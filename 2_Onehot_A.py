...
def Onehot_A( param ):
    ...
    if onehot_name == 'A_cat':
        ...
        # 获取onehot的columns
        get_index = onehot_utils.GetCategoryColums(data_path, 'count', threshold)  
    elif onehot_name in ['A_cat_xgb', 'B_cat_xgb']:
        data_name = '_'.join(onehot_name.split('_')[:2])
        print(data_name)
        X_train, y_train = onehot_utils.GetOneHot_XGB(output_path, file_name, model_path, 
                                    data_begin, num_trees, deep, data_name )
...
#    test_click:4577464
#
#   Onehot_A: ['A_cat', 'A_hour', 'A_his',
#              'A_cat_xgb','A_hour_xgb','A_his_xgb',]
#   Onehot_B: ['B_cat', 'B_cat_xgb']
# ===========================================
...
if __name__ == "__main__":
    ...
    # 多进程处理onehot
    model_name = PARAMS.onehot_name.split('_')[-1]
    if model_name=='xgb':
        for p in param:
            pass
            Onehot_A(p)
